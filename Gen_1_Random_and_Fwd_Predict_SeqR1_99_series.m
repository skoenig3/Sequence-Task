% Generates sequenes for seqR01-99
% modified from "Gen_one_rand_one_predict.m" by Seth Konig on 5/18/15
...
rand('seed',150815); %don't modify so can regenerate the same sequences
for i = 1:1000;
...
for itm = 1:99
    ...
    if itm < 10
        set = ['SeqR0' num2str(itm)];
    else
        set = ['SeqR' num2str(itm)];
    end
    fid = fopen([set '.itm'],'w+');
