...
% sequence_cortex_files = {'TT140918.1','TT140922.1','TT140923.1','TT140924.1',...
%     'TT140929.1','TT140930.1'};%,'TT141001.1'};
% calibration_cortex_files = {'TT140918.2','TT140922.2','TT140923.2','TT140924.2',...
%     'TT140929.2','TT140930.2','TT141001.1'};
% item_letter = ['G','G','H','I','J','J','K','L'];
% number_calibration_points = [26 26 26 26 26 26 26];
...
% item_letter = ['G','H','J','I','K','L'];
%  number_calibration_points = 25*ones(1,6);
...
eye_tracking_error = 4; %dva of fixation window
clr = ['bycy'];
...
%                     ylim([-14 14])
            ...
            for c = 1:size(crosses,2)
                ...
                valid_ind(valid_ind > length(xy)) = [];
                 ...
                 C = intersect(saccadetimes,valid_ind);
                 for s = 1:length(saccade_amplitudes);
                     if saccade_amplitudes(s) < 2.5
                         if saccadetimes(1,s) >= valid_ind(1) && saccadetimes(2,s) <= valid_ind(end);
                             sx = xy(1,saccadetimes(1,s):saccadetimes(2,s));
                             sy = xy(1,saccadetimes(1,s):saccadetimes(2,s));
                             dist = sqrt((sx-crosses(1,c)).^2+(sy-crosses(2,c)).^2);
                             if any(dist < 2.5);
                                 cd = [cd saccadetimes(2,s)-saccadetimes(1,s)+1];
                                 if fixationtimes(1) == 1 && size(fixationtimes,2) >= size(saccadetimes,2)
                                    priorfixdur = [priorfixdur fixationtimes(2,s)-fixationtimes(1,s)];
                                 else
                                    priorfixdur = [priorfixdur fixationtimes(2,s-1)-fixationtimes(1,s-1)]; 
                                 end
                             end
                         end
                     end
                ...
                if ~isempty(potential_fix)
                    ...
                    if max(cell2mat(crosshair_locations(2,:))) == 2
...
%%
% how do reaction times during random sequences change with trial or time
...
plot(1:length(t2f),p(1)*(1:length(t2f))+p(2),'r')
%%
% are reaction times during random sequences different for different items
